...
public final class ProcessLock implements Closeable {
    ...
    static {
        IOUtil.deleteFileOrDir(CommonUtilsEnv.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE));
    }
    ...
    public static ProcessLock tryLock(final String lockName, final boolean writeMode) {
        synchronized (LOCK_MAP) {
            ...
            try {
                File file = new File(CommonUtilsEnv.app().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE), customHash(lockName));
     ...
     * @param lockName
     * @param writeMode         是否写入模式(支持读并�?.
     * @param maxWaitTimeMillis �?���?1000 * 60
     * @return null �?进程�? 如果锁已经被占用, 则在超时时间内继续尝试获取该�?
     */
    ...
    /**
     * 取得字符串的自定义hash�? 尽量保证255字节内的hash不重�?
     *
     * @param str
     * @return
     */
