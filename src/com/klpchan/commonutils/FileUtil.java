...
public class FileUtil {
        ...
        if (existsSdcard()) {
            ...
            if (cacheDir == null) {
                result = new File(Environment.getExternalStorageDirectory(),
                        ".android/" + app.getPackageName() + "/" + dirName);
            } else {
                result = new File(cacheDir, dirName);
            }
        } else {
            result = new File(app.getFilesDir(), dirName);
        }
    ...
    public static long getFileOrDirSize(File file) {
        ...
        if (list != null) { // 文件夹被删除�? 子文件正在被写入, 文件属�?异常返回null.
            for (File item : list) {
    ...
    /**
     * 复制文件到指定文�?     *
     * @param fromPath 源文�?     * @param toPath   复制到的文件
